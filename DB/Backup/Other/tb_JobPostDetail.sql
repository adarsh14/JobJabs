...
AS
 BEGIN
  ...
  IF(@QueryType=2)
   ...
   IF(@QueryType=3)
	  BEGIN 
	     SELECT a.FranchiseId, FranchiseName,(CASE WHEN ISNULL(c.FranchiseId,0) = 0 THEN 'false' ELSE 'true' END) as Checked
		 FROM FranchiseDetail a
		      INNER JOIN SPOCAdminFranchiseDetail b ON (a.FranchiseId = b.FranchiseId)
		      LEFT OUTER JOIN JobPostFranchiseDetail c ON (a.FranchiseId=c.FranchiseId AND c.JPFAStatus=1 AND c.JobPostId=@JobPostId)
		 WHERE a.FranchiseStatus=1 AND b.SPFAStatus =1 AND b.SPOCAdminId=@SPOCAdminId
      END
 END
 GO

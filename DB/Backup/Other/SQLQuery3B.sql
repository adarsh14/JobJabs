  ...
  BEGIN
   ...
   END
	...
	IF(@QueryType=4)
     BEGIN
		 SELECT *,
		     (SELECT COUNT(1) FROM SPOCAdminFranchiseDetail a1 INNER JOIN FranchiseDetail b1 ON (a1.FranchiseId=b1.FranchiseId)
			   WHERE  SPFAStatus=1 AND SPOCAdminId=UserDetail.UserId AND b1.FranchiseStatus=1) as TotalAssignedFranchise

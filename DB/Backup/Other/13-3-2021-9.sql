...
AS
  BEGIN
     IF(@RecruiterId > 0)
	   BEGIN
	      SELECT DISTINCT a.JobPostId as [Value],JobTitle as [Text] ,JobLocation as [StringValue]
		  FROM JobPostDetail a
		       INNER JOIN JobPostRecruiterDetail b on (a.JobPostId=b.JobPostId)
		  WHERE b.RecruiterId=@RecruiterId 
		        AND a.ClientId=(CASE WHEN @ClientId > 0 THEN @ClientId ELSE ClientId END)
	   END
